...
实施时因第一阶段修复迁移已占用 V3，账本/预算/周期规则实际以前向迁移 V4–V6 落地；V7 增加账本成员、分类家庭/类型和支出预算分类的复合完整性约束。不得改写已经应用的 V1–V6。
...
注册不自动创建持久化预算模板：系统不能替家庭猜测预算月份或金额。家庭所有者/管理员须在注册后通过预算接口显式提交 `periodMonth`、范围和正数金额，才会创建预算记录。
...
新家庭初始没有预算记录；预算月份、范围和金额全部来自所有者或管理员的显式创建请求。
...
启动器从仓库 `V*.sql` 动态取得最新数字版本，并以只读方式把生产 H2 history 分类为 `NO_HISTORY`、`BEHIND_CURRENT`、`CURRENT` 或 `FAILED`。无 history 或落后版本必须先复制并校验全部数据库伴随文件到带时间戳的 `data-backups/`，只有当前版本跳过；失败 history 也先备份当前状态，再拒绝启动并要求人工修复数据和显式执行 Flyway `repair`，不得自动修复。未来版本或含糊 history 同样先备份再拒绝，且不得改写 history。备份目录默认不提交 Git。
