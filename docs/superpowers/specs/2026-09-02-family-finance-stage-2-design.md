...
启动器从仓库 `V*.sql` 动态取得最新数字版本，并以只读方式把生产 H2 history 分类为 `NO_HISTORY`、`BEHIND_CURRENT`、`CURRENT` 或 `FAILED`。无 history 或落后版本必须先复制并校验全部数据库伴随文件到带时间戳的 `data-backups/`，只有当前版本跳过；失败 history 也先备份当前状态，再拒绝启动并要求人工修复数据和显式执行 Flyway `repair`，不得自动修复。未来版本或含糊 history 同样拒绝启动。备份目录默认不提交 Git。
