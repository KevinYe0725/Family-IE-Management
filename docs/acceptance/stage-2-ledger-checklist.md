...
- `scripts/unix-startup-gates.sh`：macOS/Linux 的 13 个隔离启动、备份、迁移状态、修复重试、恢复和失败保护场景；其中单独验证 V6 待迁移先备份、V7 当前态跳过、失败 V7 先备份再拒绝，以及修复数据并执行 Flyway `repair` 后无陈旧守卫地成功重试。
