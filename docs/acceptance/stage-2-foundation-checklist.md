...
| 旧库先备份再迁移 | 在 Windows 上放入非空主库 `data/family-finance.mv.db`（或旧式 `family-finance.h2.db`）且不含 Flyway history，运行 `start-local.cmd -NoBrowser`。在应用开始迁移前，确认出现 `data-backups/<时间戳>/`，其中完整复制全部 `family-finance.*.db`（包括零字节跟随文件），并有 `RESTORE.txt`。 |
