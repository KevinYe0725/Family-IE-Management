...
首次启动会写入演示家庭及 2026 年 6—9 月账目。默认数据文件是 `data/family-finance.mv.db`（H2 还可能创建同目录的跟随文件）。
...
所有写操作都需要先从 `/api/csrf` 取得令牌，并在 `X-XSRF-TOKEN` 请求头中发送。浏览器界面会在每次写入后重新读取服务器的权威状态。
