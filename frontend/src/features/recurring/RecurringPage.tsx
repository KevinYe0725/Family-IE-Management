...
const staleReview = (error:unknown) => error instanceof ApiError && (error.code === 'RECURRING_CONFIRMATION_STALE' || error.status === 404);
...
export function RecurringPage({ request, role, userId }: { request: RequestFn; role: HouseholdRole; userId: number }) {
  ...
  const [selectedTokens,setSelectedTokens] = useState<Record<number,string>>({});
  const [payment, setPayment] = useState<(RecurringOccurrence & {ruleSnapshot:RecurringRule}) | null>(null);
  ...
  const selectableItems = pendingItems.filter(item => item.assignedUserId === userId && ruleMap.get(item.ruleId)?.confirmationToken);
  ...
  const confirm = useMutation({ mutationFn: (id: number) => request<RecurringOccurrence>(`/api/recurring-occurrences/${id}/confirm`, { method: 'POST', body: { amount: paymentAmount, confirmationToken: payment?.ruleSnapshot.confirmationToken } }), onError: fundsError, onSuccess: () => { setPayment(null); setPaymentAmount(''); refreshRecurring(); } });
  const confirmBatch = useMutation({ mutationFn: (ids: number[]) => request('/api/recurring-occurrences/confirm', { method: 'POST', body: { occurrenceIds: ids, confirmationTokens:Object.fromEntries(ids.map(id=>[id,selectedTokens[id]])) } }), onError: fundsError, onSuccess: () => { setSelectedOccurrenceIds([]); setSelectedTokens({}); refreshRecurring(); } });
  ...
  const openPayment = (item: RecurringOccurrence) => { const rule=ruleMap.get(item.ruleId); if(!rule?.confirmationToken)return; setPayment({...item,ruleSnapshot:{...rule}}); setPaymentAmount(rule.amount); confirm.reset(); };
  const skipFromDetail = () => { if (!payment || skip.isPending) return; const id = payment.id; setPayment(null); skip.mutate(id); };
  const toggleSelected = (id: number) => {
    const item=pendingItems.find(item=>item.id===id), token=item&&ruleMap.get(item.ruleId)?.confirmationToken;
    if(!token)return;
    setSelectedTokens(tokens=>({...tokens,[id]:token}));
    setSelectedOccurrenceIds(ids => ids.includes(id) ? ids.filter(item => item !== id) : [...ids, id]);
  };
  const toggleAll = () => { setSelectedOccurrenceIds(allSelected ? [] : selectableItems.map(item => item.id)); setSelectedTokens(allSelected?{}:Object.fromEntries(selectableItems.map(item=>[item.id,ruleMap.get(item.ruleId)!.confirmationToken!]))); };
  const changeOccurrencePage = (page: number) => { setSelectedOccurrenceIds([]); setSelectedTokens({}); setOccurrencePage(page); };
  const paymentRule = payment?.ruleSnapshot;
  const reviewAgain=()=>{setPayment(null);setPaymentAmount('');setSelectedOccurrenceIds([]);setSelectedTokens({});confirm.reset();confirmBatch.reset();refreshRecurring();};
  ...
  return <PageScaffold title="周期账单" description="把固定收入与支出变成可确认的计划，到期后由你决定何时入账。" primaryAction={manager ? { label: '新建周期规则', onClick: () => setDraft(newRule()) } : undefined}>
    ...
    <FormError error={(!payment && confirm.error) || confirmBatch.error || skip.error || archive.error} />
    {staleReview(confirmBatch.error)&&<Button onClick={reviewAgain}>刷新账单后重新选择</Button>}
    ...
    <ModalDialog open={payment !== null} title="账单详情" description="确认前请核对本期账单信息" className="recurring-detail-dialog" onClose={() => { setPayment(null); setPaymentAmount(''); confirm.reset(); }} footer={<><Button loading={skip.isPending} onClick={skipFromDetail}>跳过本期</Button><Button theme="solid" type="primary" loading={confirm.isPending} disabled={payment?.assignedUserId !== userId || !paymentRule?.confirmationToken || staleReview(confirm.error)} onClick={() => payment && confirm.mutate(payment.id)}>记录本次账单</Button></>}>{payment && <><FormError error={confirm.error} />{staleReview(confirm.error)&&<Button onClick={reviewAgain}>重新核对账单</Button>}<div className="recurring-detail-summary"><CategoryIcon name={paymentRule?.categoryName} kind={paymentRule?.kind} /><div><strong>{paymentRule?.categoryName ?? `规则 #${payment.ruleId}`}</strong><span>{overdueDays(payment.dueOn) > 0 ? `逾期 ${overdueDays(payment.dueOn)} 天` : '待确认'}</span></div><strong aria-label={`${kindLabel(paymentRule?.kind)}金额`} className={`recurring-amount ${paymentRule?.kind ?? ''}`}>{kindSign(paymentRule?.kind)}{money(paymentRule?.amount)}</strong></div><dl className="recurring-detail-list"><div><dt>到期日</dt><dd>{payment.dueOn}</dd></div><div><dt>账户</dt><dd>{paymentRule?.accountName ?? '账户信息不可用'}</dd></div><div><dt>归属成员</dt><dd>{paymentRule?.memberName ?? '未分配'}</dd></div><div><dt>确认人</dt><dd>{memberships.data?.find(member=>member.userId===payment.assignedUserId)?.displayName ?? (payment.assignedUserId===paymentRule?.assignedUserId?paymentRule.assignedUserName:'待核对')}</dd></div><div><dt>周期</dt><dd>{paymentRule ? scheduleText(paymentRule) : '规则信息不可用'}</dd></div></dl><div className="recurring-detail-amount"><label htmlFor="paymentAmount">本期实际金额</label><input id="paymentAmount" name="paymentAmount" required inputMode="decimal" value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} /><p>修改金额只影响本期账单，不会改变后续周期规则。</p></div><PaymentPreview account={accounts.data?.find(a => a.id === paymentRule?.accountId)} amount={paymentAmount} incoming={paymentRule?.kind === 'income'} showNote={false} /></>}</ModalDialog>
  </PageScaffold>;
}
