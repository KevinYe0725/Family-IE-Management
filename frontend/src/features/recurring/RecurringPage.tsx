...
function RecurringOverview({ pendingCount, selectableCount, activeRules, loading }: {
  ...
  return <section className="recurring-overview" aria-label="周期账单概览">
    ...
    <div className="recurring-overview-main">
      <div className="recurring-expense-analysis"><div className="recurring-expense-analysis-heading"><span>分类分析</span><h3>支出分类</h3>{topCategory && <p>支出分类中，<strong>{topCategory.name}</strong>占比最高，共 <em>{(topCategory.amount / categoryTotal * 100).toFixed(1)}%</em>。</p>}</div>{categoryData.length ? <svg className="recurring-category-ring" viewBox="0 0 620 360" role="img" aria-label={`支出分类：${categorySegments.map(item => `${item.name} ${item.percentage.toFixed(1)}%`).join('，')}`}><circle className="recurring-category-ring-track" cx="310" cy="180" r="170" />{categorySegments.map(item => { const fullRing = item.endAngle - item.startAngle >= 359.9; const path = fullRing ? '' : categoryArcPath(310, 180, 160, 95, item.startAngle, item.endAngle); const midAngle = fullRing ? 0 : (item.startAngle + item.endAngle) / 2; const anchor = categoryPolarPoint(310, 180, 178, midAngle); const layout = categoryLabelLayout.get(item.name) ?? { right: anchor.x >= 310, labelY: anchor.y }; const bendX = layout.right ? 525 : 95; const textX = layout.right ? 565 : 55; return <g key={item.name}>{fullRing ? <circle cx="310" cy="180" r="128" fill="none" stroke={item.color} strokeWidth="64" /> : <path className="recurring-category-ring-segment" d={path} fill={item.color} />}<path className="recurring-category-ring-leader" d={`M ${anchor.x.toFixed(1)} ${anchor.y.toFixed(1)} L ${bendX} ${anchor.y.toFixed(1)} L ${textX} ${layout.labelY}`} /><circle className="recurring-category-ring-dot" cx={anchor.x} cy={anchor.y} r="2.5" fill={item.color} /><text className="recurring-category-ring-label" x={layout.right ? textX + 6 : textX - 6} y={layout.labelY} textAnchor={layout.right ? 'start' : 'end'} dominantBaseline="middle"><tspan>{item.name} </tspan><tspan className="recurring-category-ring-value">{item.percentage.toFixed(1)}%</tspan></text></g>; })}<circle className="recurring-category-ring-hole" cx="310" cy="180" r="95" /><text className="recurring-category-ring-center-name" x="310" y="170" textAnchor="middle" dominantBaseline="middle">{topCategory?.name ?? '暂无'}</text><text className="recurring-category-ring-center-value" x="310" y="202" textAnchor="middle" dominantBaseline="middle">{topCategory ? `${(topCategory.amount / categoryTotal * 100).toFixed(1)}%` : '—'}</text></svg> : <p className="recurring-chart-empty">{loading ? '正在读取周期规则…' : '暂无执行中的支出规则'}</p>}</div>
