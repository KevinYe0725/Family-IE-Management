...
it('does not call unknown actual spending zero when historic FX is missing', async () => {
 const request = (async (path:string) => {
  ...
  if(path.startsWith('/api/dashboard'))throw new ApiError('缺少汇率',{status:409,code:'FX_RATE_MISSING'});
 ...
 expect(await screen.findByText('实际总支出 —')).toBeInTheDocument();
 expect(screen.queryByText('实际总支出 ¥0.00')).not.toBeInTheDocument();
