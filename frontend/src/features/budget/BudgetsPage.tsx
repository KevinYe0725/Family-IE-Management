...
export function BudgetsPage({ request, role }: { request: RequestFn; role: HouseholdRole }) {
  ...
  const dashboard = useQuery({ queryKey: ['dashboard', month], queryFn: () => request<{ summary: { expense?: string | null } }>(`/api/dashboard?month=${month}`) });
  ...
  const actualSpent = dashboard.data?.summary?.expense ? parseCents(dashboard.data.summary.expense) : 0n;
  ...
  return <PageScaffold title="费用预算" description="顶部月度总预算为家庭总额；分类预算计入总额并受其约束，成员项为观察线、不占总额。" primaryAction={manager ? { label: '新建预算', onClick: () => setDraft({ periodMonth: month, scopeType: 'CATEGORY', categoryId: null, memberId: null, amount: '', version: 0, active: true, note: '' }) } : undefined} readonly={!manager}>
    {manager && <section className="budget-total-bar" aria-label="月度总预算">
      <div className="budget-total-head"><strong>月度总预算</strong>
        {totalAmount === null
      ...
      {totalAmount !== null && <>
        <div className="budget-total-values"><span>总额 {money(formatYuan(totalAmount))}</span><span>已分配 {money(formatYuan(allocated))}</span><span>剩余可分配 {money(formatYuan(remaining))}</span><span>实际总支出 {money(formatYuan(actualSpent))}</span></div>
        ...
        {actualSpent > totalAmount && <p role="alert" className="field-help">本月实际总支出已超过总预算。</p>}
      </>}
      {totalAmount === null && <p className="field-help">尚未设置本月总预算：设置后分类预算的“已分配”合计将受总预算约束。</p>}
    </section>}
    <div className="toolbar budget-toolbar"><label>预算月份<input type="month" value={month} onChange={e => setMonth(e.target.value)} /></label>
