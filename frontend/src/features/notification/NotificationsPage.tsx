...
import type { HouseholdRole, NotificationItem, NotificationPage } from '../../api/contracts';
import { DataPanel, PageScaffold, QueryState, FormError, StatusTag, dateText, isManager, type RequestFn } from '../common';

export function NotificationsPage({ request, role }: { request: RequestFn; role: HouseholdRole }) {
  ...
  const update = useMutation({ mutationFn: ({ id, action }: { id: number; action: 'read' | 'resolve' }) => request<NotificationItem>(`/api/notifications/${id}/${action}`, { method: 'POST' }) });
  const generate = useMutation({ mutationFn: () => request<number>('/api/notifications/generate', { method: 'POST' }) });
  const items = notifications.data?.items.filter(item => filter === 'all' || filter === 'unread' ? filter === 'all' || !item.readAt : !item.resolvedAt) ?? [];
  const route = (item: NotificationItem) => item.referenceType==='INVESTMENT_PLAN_OCCURRENCE' ? '/workspace/investments?tab=plans' : item.referenceType.includes('LOAN') ? '/workspace/loans' : item.referenceType.includes('ASSET') ? '/workspace/assets' : item.referenceType.includes('RECURRING') ? '/workspace/recurring' : item.referenceType.includes('BUDGET') ? '/workspace/budgets' : '/workspace/overview';
  return <PageScaffold title="提醒中心">
    <FormError error={update.error || generate.error}/>
    <div className="summary-strip notification-summary"><div><span>未读提醒</span><strong>{notifications.data?.unreadCount ?? '—'}</strong><small>当前账号未读</small></div><div><span>待处理</span><strong>{notifications.data?.items.filter(item => !item.resolvedAt).length ?? '—'}</strong><small>当前页面记录</small></div>{isManager(role) && <Button size="small" loading={generate.isPending} onClick={() => generate.mutate()}>刷新今日提醒</Button>}</div>
