...
export function TransactionsPage({ request, role, userId, requestedSection }: { request: RequestFn; role: HouseholdRole; userId: number; requestedSection?: 'accounts' }) {
  ...
  const selectKind = (value:TransactionKind) => {setKind(value);setCategoryId('');setSelectedDay(null);};
  const [q, setQ] = useState('');
  const [accountId, setAccountId] = useState('');
  const [bankAccountId,setBankAccountId]=useState('');
  const [exchangeBankId,setExchangeBankId]=useState<number|null>(null);
  const [transferKind,setTransferKind]=useState<'same'|'fx'>('same');
  ...
  function newTransaction() {
    ...
    setDraft({ ...emptyDraft(), kind, accountId: eligible.length === 1 ? String(eligible[0].id) : '', memberId: members.data?.length === 1 ? String(members.data[0].id) : '' });
  }
  ...
  const canEdit = (item: Transaction) => item.cashImpact !== false && (manager || item.createdByUserId === userId);
  ...
  return <PageScaffold title="收支明细" primaryAction={{label:"记一笔",onClick:newTransaction}} className="ledger-workspace">
    ...
    <nav className="segmented-tabs" aria-label="流水视角"><button type="button" className={viewMode==='records'?'active':''} onClick={()=>setViewMode('records')}>收支记录</button><button type="button" className={viewMode==='cash'?'active':''} onClick={()=>setViewMode('cash')}>资金流水</button></nav>
    {viewMode==='cash'&&<CashMovementsPanel request={request} accounts={accountOptions.data??[]} month={month} onMonth={setMonth}/>}
    {viewMode==='records'&&<><LedgerCharts categories={flatCategories} request={request} filters={filters} kind={kind} selectedDay={selectedDay} onKind={selectKind} onCategory={id=>{setCategoryId(id?String(id):'');setSelectedDay(null);}} onDay={setSelectedDay}/>
    <>
      <div className="filter-bar">
        <label>账期<DateField aria-label="账期" mode="month" allowClear={false} value={month} onChange={e => setMonth(e.target.value)} /></label>
        <label>类型<select aria-label="收支类型筛选" value={kind} onChange={e => selectKind(e.target.value as TransactionKind)}><option value="expense">支出</option><option value="income">收入</option></select></label>
<label>银行卡<select aria-label="银行卡筛选" value={bankAccountId} onChange={e=>{setBankAccountId(e.target.value);setAccountId("");}}><option value="">全部</option>{[...new Map((accountOptions.data??[]).filter(a=>a.bankAccountId).map(a=>[a.bankAccountId!,a.bankAccountName??a.name])).entries()].map(([id,name])=><option key={id} value={id}>{name}</option>)}</select></label>
      ...
      <QueryState loading={transactions.isLoading || accountOptions.isLoading || categoryOptions.isLoading || members.isLoading} error={transactions.error || accountOptions.error || categoryOptions.error || members.error} empty={transactions.data?.items.length === 0 && transactionPage === 0} emptyTitle="还没有收支记录" emptyDetail="点击“记一笔”开始记录家庭现金流。">
        <>
          <div className="responsive-data"><table><thead><tr><th>日期</th><th>类型</th><th>金额</th><th>分类</th><th>账户 / 结算方式</th><th>成员</th><th>创建人</th><th>商家 / 备注</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{transactions.data?.items.map(item => <tr key={item.id}><td>{item.occurredOn}</td><td><StatusTag tone={item.kind === 'income' ? 'success' : 'blue'}>{item.cashImpact === false ? '非现金还款' : item.kind === 'income' ? '收入' : item.principalAmount != null ? '还款现金流出' : '费用支出'}</StatusTag></td><td className={`money${item.cashImpact === false ? '' : ` ${item.kind}`}`}>{item.cashImpact === false ? '' : item.kind === 'expense' ? '-' : '+'}{money(item.amount,item.currency)}{item.principalAmount != null && <small>本金 {money(item.principalAmount)} · 利息 {money(item.interestAmount)}</small>}</td><td>{item.categoryName}</td><td>{item.cashImpact === false ? '买方代偿（非现金）' : item.accountName}</td><td>{item.memberName}</td><td>{item.createdByName || '—'}</td><td><strong>{item.merchant || '—'}</strong><small>{item.note}</small></td><td><button className="text-action" onClick={() => setDetail(item)}>详情</button>{canEdit(item) && <button className="text-action" onClick={() => editTransaction(item)}>编辑</button>}{canDelete(item) && <button className="text-action danger" onClick={() => setDeleteId(item.id)}>删除</button>}</td></tr>)}</tbody></table>
            <div className="mobile-card-list">{transactions.data?.items.map(item => <article className="record-card" key={item.id}><header><div><StatusTag tone={item.kind === 'income' ? 'success' : 'blue'}>{item.cashImpact === false ? '非现金还款' : item.kind === 'income' ? '收入' : item.principalAmount != null ? '还款现金流出' : '费用支出'}</StatusTag><strong>{item.categoryName}</strong></div><b>{item.cashImpact === false ? '' : item.kind === 'expense' ? '-' : '+'}{money(item.amount,item.currency)}</b></header><p>{item.occurredOn} · {item.cashImpact === false ? '买方代偿（非现金）' : item.accountName} · {item.memberName} · 创建人：{item.createdByName || '—'}</p><footer><span>{item.merchant || item.note || '无备注'}{item.principalAmount != null && <small>本金 {money(item.principalAmount)} · 利息 {money(item.interestAmount)}</small>}</span><span><button onClick={() => setDetail(item)}>详情</button>{canEdit(item) && <button onClick={() => editTransaction(item)}>编辑</button>}{canDelete(item) && <button onClick={() => setDeleteId(item.id)}>删除</button>}</span></footer></article>)}</div>
          </div>
