...
export function TransactionsPage({ request, role }: { request: RequestFn; role: HouseholdRole; userId: number }) {
  ...
  const transactions = useQuery({ queryKey: ['transactions', filters, transactionPage], queryFn: () => request<Transaction[]>(`/api/transactions?${filters}&page=${transactionPage}&size=50`) });
  const accounts = useQuery({ queryKey: ['accounts'], queryFn: () => request<{ items: Account[] }>('/api/accounts?page=0&size=50') });
  const categories = useQuery({ queryKey: ['categories', categoryPage], queryFn: () => request<Category[]>(`/api/categories?projection=tree&page=${categoryPage}&size=50`) });
  const members = useQuery({ queryKey: ['members'], queryFn: () => request<Member[]>('/api/members') });
  const orderedCategories = useMemo(() => (categories.data ?? []).map(item => ({
    ...item,
    children: [...(item.children ?? [])].sort((left, right) => left.id - right.id)
  })).sort((left, right) => left.id - right.id), [categories.data]);
  const flatCategories = useMemo(() => orderedCategories.flatMap(item => [item, ...(item.children ?? [])]), [orderedCategories]);
  ...
  return <PageScaffold title="收支明细" description="账户、分类和每一笔家庭现金流，都以服务器记录为准。" primaryAction={action}>
    ...
    {section === 'transactions' && <>
      <div className="filter-bar">
        ...
        <a className="secondary-action" href={exportHref} download>导出 CSV</a>
      </div>
      <QueryState loading={transactions.isLoading || accounts.isLoading || categories.isLoading || members.isLoading} error={transactions.error || accounts.error || categories.error || members.error} empty={transactions.data?.length === 0 && transactionPage === 0} emptyTitle="还没有收支记录" emptyDetail="点击“记一笔”开始记录家庭现金流。">
        <>
          <div className="responsive-data"><table><thead><tr><th>日期</th><th>类型</th><th>金额</th><th>分类</th><th>账户</th><th>成员</th><th>创建人</th><th>商家 / 备注</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{transactions.data?.map(item => <tr key={item.id}><td>{item.occurredOn}</td><td><StatusTag tone={item.kind === 'income' ? 'success' : 'blue'}>{item.kind === 'income' ? '收入' : '支出'}</StatusTag></td><td className={`money ${item.kind}`}>{item.kind === 'expense' ? '-' : '+'}{money(item.amount)}</td><td>{item.categoryName}</td><td>{item.accountName}</td><td>{item.memberName}</td><td>{item.createdByName || '—'}</td><td><strong>{item.merchant || '—'}</strong><small>{item.note}</small></td><td><button className="text-action" onClick={() => editTransaction(item)}>编辑</button><button className="text-action danger" onClick={() => setDeleteId(item.id)}>删除</button></td></tr>)}</tbody></table>
          ...
          <PaginationControls page={transactionPage} hasNext={(transactions.data?.length ?? 0) === 50} onPageChange={setTransactionPage} label="收支记录" />
        </>
      </QueryState>
    </>}
    ...
    <Drawer open={Boolean(categoryDraft)} title={categoryDraft?.id ? '编辑分类' : '新建分类'} onClose={() => setCategoryDraft(null)}>{categoryDraft && <form className="feature-form" onSubmit={e => { e.preventDefault(); saveCategory.mutate(categoryDraft); }}><FormError error={saveCategory.error} /><label>分类名称<input required value={categoryDraft.name} onChange={e => setCategoryDraft({ ...categoryDraft, name: e.target.value })} /></label><label>收支类型<select value={categoryDraft.kind} onChange={e => setCategoryDraft({ ...categoryDraft, kind: e.target.value as TransactionKind })}><option value="expense">支出</option><option value="income">收入</option></select></label><label>上级分类<select value={categoryDraft.parentId} onChange={e => setCategoryDraft({ ...categoryDraft, parentId: e.target.value })}><option value="">一级分类</option>{orderedCategories.filter(item => item.kind === categoryDraft.kind && item.id !== categoryDraft.id).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>标记颜色<input type="color" value={categoryDraft.color} onChange={e => setCategoryDraft({ ...categoryDraft, color: e.target.value })} /></label><Button htmlType="submit" theme="solid" type="primary" loading={saveCategory.isPending}>保存分类</Button></form>}</Drawer>
    <ConfirmDialog open={deleteId !== null} title="删除这笔收支？" detail={<><p>该操作会重新计算相关预算和总览，且无法撤销。</p><FormError error={remove.error} /></>} danger confirmLabel="删除收支" onClose={() => { setDeleteId(null); remove.reset(); }} onConfirm={() => deleteId !== null && remove.mutate(deleteId)} />
  </PageScaffold>;
}

function PaginationControls({ page, hasNext, onPageChange, label }: { page: number; hasNext: boolean; onPageChange: (page: number) => void; label: string }) {
  if (page === 0 && !hasNext) return null;
  return <nav className="pagination-controls" aria-label={`${label}分页`}><button type="button" disabled={page === 0} onClick={() => onPageChange(Math.max(0, page - 1))}>上一页</button><span>第 {page + 1} 页</span><button type="button" disabled={!hasNext} onClick={() => onPageChange(page + 1)}>下一页</button></nav>;
}
