...
import { accountDescription } from './account-label';
import './account-identity.scss';

// Simple Icons CC0 paths. See docs/design/account-icons.md for provenance.
...
type Identity=Pick<Account,'type'|'walletProvider'|'bankName'|'cardLastFour'>;
export function AccountIcon({account}:{account:Identity}){
  const brand=account.type==='WALLET'?account.walletProvider:null;
  return <span className={`account-emblem account-emblem--${brand?.toLowerCase()??account.type?.toLowerCase()??'unknown'}`} aria-hidden="true">
    {brand==='ALIPAY'||brand==='WECHAT'?<svg viewBox="0 0 24 24" fill="currentColor"><path d={brand==='ALIPAY'?alipay:wechat}/></svg>:
