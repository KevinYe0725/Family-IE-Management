...
export function BankAccountsPanel({request,manager,currencies,onOpening,onHistory,onExchange,onOverlayChange}:{request:RequestFn;manager:boolean;currencies:string[];onOpening:(account:Account)=>void;onHistory:(bankId:number)=>void;onExchange:(bankId:number)=>void;onOverlayChange?:(open:boolean)=>void}){
 ...
 useEffect(()=>{onOverlayChange?.(selected!==null||editor!==null||archiving!==null);},[selected,editor,archiving,onOverlayChange]);
 ...
 return <section className="bank-management" aria-label="银行卡账户"><div className="panel-heading"><h2>银行卡</h2>{manager&&<Button theme="solid" type="primary" icon={<Plus size={17} aria-hidden="true"/>} onClick={()=>setEditor({mode:'create'})}>新建银行卡</Button>}</div>
