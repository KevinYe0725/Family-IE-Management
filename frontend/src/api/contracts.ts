...
export interface Loan { latestStrategy?: PrepaymentStrategy | null; remainingTerm?: number; maturityOn?: string | null; nextPaymentOn?: string | null; nextPaymentAmount?: string | null; scheduledRepaymentTotal?: string; remainingRepaymentTotal?: string; paidRepaymentTotal?: string; purchasedAssetId?: number | null; fundingMode?: 'OPENING' | 'DISBURSEMENT' | 'FINANCED_PURCHASE' | null; accountingOn?: string | null; disbursementAccountId?: number | null; accountingInitialized?: boolean; lastPaymentOn?: string | null; id: number; name: string; type: 'MORTGAGE' | 'CAR' | 'OTHER'; linkedAssetId: number | null; memberId: number | null; assignedUserId: number | null; paymentAccountId: number; paymentCategoryId: number; principal: string; annualRate: string; termMonths: number; repaymentMethod: 'EQUAL_PAYMENT' | 'EQUAL_PRINCIPAL' | 'CUSTOM'; startOn: string; currentPrincipal: string; status: 'ACTIVE' | 'ARCHIVED' | 'CLOSED' }
...
export interface LoanDebtOverview { count: number; remainingPrincipal: string; remainingRepayment: string; thirtyDayDue: string; paidRepayment: string; weightedAnnualRatePercent: string; nextDueOn: string | null }
