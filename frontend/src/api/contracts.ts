...
export interface LoanRepayment extends LoanRepaymentPreview {
 cashImpact?: boolean; settlementAssetId?: number | null;
...
export interface Loan { purchaseValue?: string | null; ownContribution?: string; ownContributionAccountId?: number | null; assetRelation?: 'FINANCING' | 'COLLATERAL' | null; linkedAssetName?: string | null; disbursementAmount?: string | null; withheldFee?: string; latestStrategy?: PrepaymentStrategy | null; remainingTerm?: number; maturityOn?: string | null; nextPaymentOn?: string | null; nextPaymentAmount?: string | null; scheduledRepaymentTotal?: string; remainingRepaymentTotal?: string; paidRepaymentTotal?: string; purchasedAssetId?: number | null; fundingMode?: 'OPENING' | 'DISBURSEMENT' | 'FINANCED_PURCHASE' | null; accountingOn?: string | null; disbursementAccountId?: number | null; accountingInitialized?: boolean; lastPaymentOn?: string | null; overdueInstallments?: number; overdueAmount?: string; overdueDays?: number; id: number; name: string; type: 'MORTGAGE' | 'CAR' | 'OTHER'; linkedAssetId: number | null; memberId: number | null; assignedUserId: number | null; paymentAccountId: number; paymentCategoryId: number; principal: string; annualRate: string; termMonths: number; repaymentMethod: 'EQUAL_PAYMENT' | 'EQUAL_PRINCIPAL' | 'CUSTOM'; startOn: string; currentPrincipal: string; status: 'ACTIVE' | 'ARCHIVED' | 'CLOSED' }
