...
public final class MigrationStateFixtureCli {
    ...
    public static void main(String[] args) throws Exception {
        ...
        switch (args[1]) {
            ...
            case "assert-version-9" -> assertVersionNine(url);
    ...
    private static void assertVersionNine(String url) throws Exception {
        try (var connection = DriverManager.getConnection(url, "sa", "");
                var rows = connection.createStatement().executeQuery(
                        "select count(*) from \"flyway_schema_history\" "
                                + "where \"version\"='9' and \"success\"=true")) {
            rows.next();
            if (rows.getLong(1) != 1) throw new AssertionError("Expected one successful V9 row");
        }
    }
