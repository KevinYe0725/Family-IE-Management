...
class EncodedAuthenticationFilterIntegrationTest {
    ...
    @Test
    void encodedLoginFinalSegmentSharesCanonicalQuota() throws Exception {
        assertEncodedLoginSharesCanonicalQuota(
                "encoded-final-login@example.com", "/api/auth/%6cogin");
    }

    @Test
    void encodedApiSegmentSharesCanonicalQuota() throws Exception {
        assertEncodedLoginSharesCanonicalQuota(
                "encoded-api-login@example.com", "/a%70i/auth/login");
    }

    @Test
    void suspendedSessionIgnoresNonApiButIsInvalidatedOnEncodedApiRequest() throws Exception {
        HttpClient client = newClient();
        Csrf csrf = csrf(client, "/api/csrf");
        assertThat(login(client, "/api/auth/login", "demo", "demo1234", csrf).statusCode()).isEqualTo(200);
        csrf = csrf(client, "/api/csrf");
        jdbc.update("update app_users set status='SUSPENDED' where email='demo@local.family'");

        HttpResponse<String> nonApi = send(client, HttpRequest.newBuilder(uri("/"))
                .GET().build());
        assertThat(nonApi.statusCode()).isEqualTo(200);
        assertThat(nonApi.headers().firstValue("X-Request-ID")).isEmpty();

        jdbc.update("update app_users set status='ACTIVE' where email='demo@local.family'");
        HttpResponse<String> sessionAfterNonApi = send(client, HttpRequest.newBuilder(uri("/api/session"))
                .GET().build());
        assertThat(sessionAfterNonApi.statusCode()).isEqualTo(200);
        jdbc.update("update app_users set status='SUSPENDED' where email='demo@local.family'");

        HttpResponse<String> denied = send(client, HttpRequest.newBuilder(uri("/a%70i/auth/change-password"))
                .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                .header(csrf.headerName(), csrf.token())
                .POST(HttpRequest.BodyPublishers.ofString("{}"))
                .build());
        assertInvalidated(denied);

        HttpResponse<String> clearedSession = send(client, HttpRequest.newBuilder(uri("/api/session"))
                .GET().build());
    ...
    private void assertEncodedLoginSharesCanonicalQuota(String username, String encodedPath) throws Exception {
        ...
        for (int attempt = 0; attempt < 5; attempt++) {
            assertThat(login(client, "/api/auth/login", username, "wrong-password", csrf).statusCode())
                    .isEqualTo(401);
        }
        assertRateLimited(login(client, "/api/auth/login", username, "wrong-password", csrf));
    ...
    private void assertInvalidated(HttpResponse<String> response) throws Exception {
        assertThat(response.statusCode()).isEqualTo(401);
        assertThat(response.headers().firstValue("X-Request-ID")).isPresent();
        assertThat(json(response).path("error").path("code").asText()).isEqualTo("AUTH_REQUIRED");
    }
    ...
    private Csrf csrf(HttpClient client, String path) throws Exception {
        JsonNode data = json(send(client, HttpRequest.newBuilder(uri(path)).GET().build())).path("data");
