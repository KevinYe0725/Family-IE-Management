...
class InvestmentAuthorizationConcurrencyApiTest {
    ...
    @Test
    void adminDemotionCompletingBeforeHouseholdLockDeniesStaleInvestmentMutation() throws Exception {
        MockHttpSession owner = login("demo", "demo1234");
        String email = "investment-race-admin@example.com";
        ...
        Mockito.doAnswer(invocation -> {
                    if (Thread.currentThread().getName().equals("investment-after-demotion")) {
        ...
        ExecutorService executor = Executors.newSingleThreadExecutor(runnable -> {
            Thread thread = new Thread(runnable, "investment-after-demotion");
            thread.setDaemon(true);
            return thread;
        });
        try {
            Future<MvcResult> result = executor.submit(() -> mvc.perform(post("/api/investment-accounts")
                            .session(admin).with(csrf()).contentType(MediaType.APPLICATION_JSON)
                            .content("{\"name\":\"降级后账户\",\"brokerName\":\"测试券商\",\"currency\":\"CNY\"}"))
                    .andReturn());
            ...
            assertThat(jdbc.queryForObject(
                    "select count(*) from investment_accounts where household_id=? and name='降级后账户'",
    ...
    private MockHttpSession join(MockHttpSession owner, String email, HouseholdRole role) throws Exception {
        ...
        mvc.perform(post("/api/auth/register").with(csrf()).contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"email":"%s","displayName":"投资管理员","password":"family-pass-2026",
