...
import org.springframework.transaction.annotation.Transactional;
 ...
 private List<AppUser> budgetRecipients(Household household,Budget budget){if(budget.getScopeType()==BudgetScopeType.MEMBER&&budget.getMember()!=null&&budget.getMember().getLinkedUser()!=null)return List.of(budget.getMember().getLinkedUser());return memberships.findByHouseholdIdOrderById(household.getId()).stream().filter(m->m.getStatus()==MembershipStatus.ACTIVE&&(m.getRole()==HouseholdRole.OWNER||m.getRole()==HouseholdRole.ADMIN)).map(HouseholdMembership::getUser).toList();}
