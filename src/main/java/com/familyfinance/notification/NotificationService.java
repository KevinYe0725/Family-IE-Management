...
import org.springframework.transaction.annotation.Transactional;
 ...
 private static final ZoneId SHANGHAI=ZoneId.of("Asia/Shanghai"); private final NotificationRepository notifications; private final CurrentMembership current; private final FamilyMutationAuthorization authorization; private final HouseholdRepository households; private final HouseholdMembershipRepository memberships; private final BudgetRepository budgets; private final com.familyfinance.accounting.LedgerReportingService transactions; private final RecurringOccurrenceRepository recurring; private final LoanInstallmentRepository installments; private final AssetRepository assets; private final AssetValuationRepository valuations; private final MarketIssueRepository marketIssues; private final Clock clock;
 NotificationService(NotificationRepository notifications,CurrentMembership current,FamilyMutationAuthorization authorization,HouseholdRepository households,HouseholdMembershipRepository memberships,BudgetRepository budgets,com.familyfinance.accounting.LedgerReportingService transactions,RecurringOccurrenceRepository recurring,LoanInstallmentRepository installments,AssetRepository assets,AssetValuationRepository valuations,MarketIssueRepository marketIssues,Clock clock){this.notifications=notifications;this.current=current;this.authorization=authorization;this.households=households;this.memberships=memberships;this.budgets=budgets;this.transactions=transactions;this.recurring=recurring;this.installments=installments;this.assets=assets;this.valuations=valuations;this.marketIssues=marketIssues;this.clock=clock;}
 ...
 @Transactional public int generateForHousehold(Household household){long h=household.getId(); LocalDate today=LocalDate.now(clock.withZone(SHANGHAI));int made=0;
  ...
  YearMonth month=YearMonth.from(today);for(Budget b:budgets.findAllByHouseholdIdAndPeriodMonthAndActiveTrue(h,month.toString())){if(!transactions.isComplete(h))continue;long spent=spent(h,b,month); NotificationType type=null;String title=null,body=null;if(spent>=b.getAmountCents()){type=NotificationType.BUDGET_LIMIT;title="预算已达上限";body="本月预算已用尽";}else if(BigInteger.valueOf(spent).multiply(BigInteger.valueOf(100)).compareTo(BigInteger.valueOf(b.getAmountCents()).multiply(BigInteger.valueOf(80)))>=0){type=NotificationType.BUDGET_NEAR_LIMIT;title="预算接近上限";body="本月预算已使用80%以上";}if(type!=null)for(AppUser recipient:budgetRecipients(household,b))made+=open(household,recipient,type,title,body,"BUDGET",b.getId(),clock.instant())?1:0;}
 ...
 private List<AppUser> budgetRecipients(Household household,Budget budget){if((budget.getScopeType()==BudgetScopeType.MEMBER||budget.getScopeType()==BudgetScopeType.CATEGORY_MEMBER)&&budget.getMember()!=null&&budget.getMember().getLinkedUser()!=null)return List.of(budget.getMember().getLinkedUser());return memberships.findByHouseholdIdOrderById(household.getId()).stream().filter(m->m.getStatus()==MembershipStatus.ACTIVE&&(m.getRole()==HouseholdRole.OWNER||m.getRole()==HouseholdRole.ADMIN)).map(HouseholdMembership::getUser).toList();}
 private boolean open(Household h,AppUser user,NotificationType type,String title,String body,String ref,long refId,Instant due){if(!notifications.currentIdentity(h.getId(),type,ref,refId,user==null?null:user.getId()).isEmpty())return false;try{notifications.saveAndFlush(new Notification(h,user,type,title,body,ref,refId,due));return true;}catch(DataIntegrityViolationException e){return false;}}
 private long spent(long h,Budget b,YearMonth month){String raw=transactions.sumBudgetExpenseCents(h,month.atDay(1),month.plusMonths(1).atDay(1),b.getScopeType().name(),b.getCategory()==null?null:b.getCategory().getId(),b.getMember()==null?null:b.getMember().getId(),true);return new BigInteger(raw).longValueExact();}
}
