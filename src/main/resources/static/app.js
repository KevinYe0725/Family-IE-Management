...
(function () {
  ...
  function renderAnalysis() { 
    const analysis = state.data.analysis || {}; renderShell(`<section class="panel"><div class="panel-head"><div><h2>规则分析</h2><p>${esc(analysis.historyStatus || '正在核对历史账目')}</p></div><span class="label">账期 ${esc(state.month)}</span></div>${insightCards(analysis.insights || [])}</section><section class="panel"><h2>用数据说话</h2><p>分析使用本月支出与此前三个有数据账期的平均值比较；当历史不足时，只展示可以确认的结论。</p></section><section class="panel"><h2>本月支出构成</h2><div id="category-chart" style="width: 100%; height: 400px;"></div></section>`); 
    renderCategoryChart(state.month);
  }
  ...
  async function renderCategoryChart(month) {
    ...
    try {
      const res = await fetch(`/api/reporting/category-stats?month=${month}`);
      ...
      chart.setOption({
        ...
        series: [{
          name: '支出分类', type: 'pie', radius: '60%',
          data: items.map(item => ({ name: item.name, value: item.value / 100 })),
