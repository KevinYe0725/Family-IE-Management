...
set created_by_user_id = coalesce(
    (
        ...
        join household_memberships linked_membership
          on linked_membership.household_id = profile.household_id
         and linked_membership.user_id = profile.linked_user_id
...
alter table financial_transactions alter column created_by_user_id set not null;
...
create table budgets (
    ...
    constraint uk_budgets_household_period_scope unique nulls not distinct
...
create table budget_revisions (
    ...
    constraint fk_budget_revisions_budget foreign key (budget_id) references budgets(id),
    constraint fk_budget_revisions_changed_by foreign key (changed_by) references app_users(id)
);

create index ix_budget_revisions_budget_changed
    on budget_revisions (budget_id, changed_at, id);
...
create table recurring_occurrences (
    ...
    constraint fk_recurring_occurrences_rule foreign key (rule_id) references recurring_rules(id),
    constraint fk_recurring_occurrences_transaction foreign key (confirmed_transaction_id)
        references financial_transactions(id),
    constraint fk_recurring_occurrences_assigned_user foreign key (assigned_user_id) references app_users(id)
);

create index ix_recurring_occurrences_assignee_status_due
    on recurring_occurrences (assigned_user_id, status, due_on, id);
create index ix_recurring_occurrences_status_due
    on recurring_occurrences (status, due_on, id);
