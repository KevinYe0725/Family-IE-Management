...
printf '%s\n' 'Gate 9/14: A V7 database is backed up before its pending V9 migrations.'
...
grep -F 'behind repository migration V9' "$pending_scenario/pending.log" >/dev/null ||
    fail 'The V7 pending-migration branch was not reported explicitly.'

printf '%s\n' 'Gate 10/14: Current V9 skips migration backup.'
...
grep -F 'is current at repository migration V9; no migration backup is required' "$restart_log" >/dev/null ||
    fail 'Current V9 restart did not take the explicit skip branch.'
...
migration_fixture "$failed_scenario" assert-version-9
...
run_refused_history_state FUTURE make-future-history assert-future-history \
    'migration newer than repository V9'
