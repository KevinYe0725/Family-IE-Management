...
function Test-BackupRestartCollisionAndRestore {
    ...
    Assert-True (Test-ProcessLogContains $Restart 'is current at repository migration V12; no migration backup is required') 'Current V12 restart did not take the explicit backup-skip branch.'
...
function Test-ExactMigrationStatesAndRecovery {
    ...
    Assert-True (Test-ProcessLogContains $Pending 'behind repository migration V12') 'V7 pending migration was not reported explicitly.'
    ...
    $RestartPort = Get-FreePort
    $Restart = Start-Launcher $PendingScenario $RestartPort 'v12-current'
    try {
        Wait-ForReady $RestartPort $Restart 180
    }
    finally {
    ...
    Assert-True (Test-ProcessLogContains $Restart 'is current at repository migration V12; no migration backup is required') 'Current V12 was not reported explicitly.'
    $CurrentBackups = @(Get-CompletedBackups $PendingBackupRoot)
    Assert-Equal 1 $CurrentBackups.Count 'Current V12 created an unnecessary backup.'
    ...
    foreach ($Case in @(
            [pscustomobject]@{
                ...
                Message = 'migration newer than repository V12'
            },
            [pscustomobject]@{
                Name = 'ambiguous'
...
try {
    ...
    Write-Host 'Gate 5/7: Exact V7, V12, failed, repaired, future, and ambiguous states are handled safely.'
